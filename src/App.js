...
function Badge({i}){ return <span style={{background:ST_COLORS[i],color:"#fff",borderRadius:20,padding:"3px 10px",fontSize:11,fontWeight:700}}>{ST_EMOJI[i]} {ST_LABELS[i]}</span>; }
  ...
  return(
    <div style={card}>
      {!editando?<>
        ...
        {resetando&&<div style={{marginTop:8}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button style={{...btnG,marginBottom:0,flex:1,padding:"9px",fontSize:13}} onClick={()=>{
              ...
              upC(n);setResetando(false);setNovaSenha("");alert(`✅ Senha de ${c.nome} resetada!`);
            }}>✅ Salvar</button>
        ...
        <label style={lbl}>Nome</label><input style={inp} value={editForm.nome} onChange={e=>setEditForm({...editForm,nome:e.target.value})}/>
        <label style={lbl}>Rua</label><input style={inp} value={editForm.rua} onChange={e=>setEditForm({...editForm,rua:e.target.value})}/>
        <div style={{display:"flex",gap:8}}>
...
function AuthCadastro({form,setForm,onSave,onBack}){
  const [err,setErr]=useState("");
  const go=()=>{
    if(!form.nome||!form.rua||!form.num||!form.bairroSel||!form.senha||!form.dica){setErr("Preencha todos os campos.");return;}
    if(form.senha!==form.senhaConf){setErr("As senhas não coincidem.");return;}
    onSave();
  };
  return(
    <div style={{padding:"14px"}}>
      ...
      <input type="password" style={inp} placeholder="Ex: gato2010, flamengo" value={form.senha} onChange={e=>setForm({...form,senha:e.target.value})}/>
      <label style={lbl}>Confirme a senha *</label>
      <input type="password" style={inp} placeholder="Digite a senha novamente" value={form.senhaConf||""} onChange={e=>setForm({...form,senhaConf:e.target.value})}/>
      {form.senha&&form.senhaConf&&<div style={{fontSize:12,marginBottom:8,color:form.senha===form.senhaConf?"#6f6":"#f55"}}>{form.senha===form.senhaConf?"✅ Senhas coincidem":"❌ Senhas não coincidem"}</div>}
      ...
      <label style={lbl}>Dica para lembrar a senha *</label>
      <input style={inp} placeholder="Ex: nome do meu gato" value={form.dica} onChange={e=>setForm({...form,dica:e.target.value})}/>
...
export default function App(){
    ...
    (async()=>{
      try{
        const [p,c,f,l,sess]=await Promise.all([
          ...
          dbGet("elchai_fotos"),dbGet("elchai_loja"),dbGet("elchai_sessao"),
        ]);
        ...
        if(sess){
          const cli=(c||[]).find(x=>x.tel===sess.tel);
          if(cli){
            setCliente(cli);setAuthTel(sess.tel);
            setEndForm(ef=>({...ef,rua:cli.rua||"",num:cli.num||"",bairroSel:cli.bairro||""}));
            if(sess.pedidoId&&p){const ped=p.find(x=>x.id===sess.pedidoId);if(ped){setPedidoConf(ped);setPedidoTs(ped.ts||null);}}
            setView("home");return;
          }
        }
  ...
  if(view==="loading") return(
    <div style={{...app,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh"}}>
      <div style={{fontSize:60,marginBottom:16}}>🥟</div>
      <div style={{fontWeight:900,fontSize:22,background:gg,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:8}}>ELCHAI</div>
      <div style={{fontSize:13,color:G.tm,marginBottom:20}}>Carregando...</div>
      <div style={{width:40,height:40,border:`3px solid ${G.gold}33`,borderTop:`3px solid ${G.gl}`,borderRadius:"50%",animation:"spin 1s linear infinite"}}/>
  ...
  if(view==="splash") return(
    <div style={app}>
      <div style={{background:`linear-gradient(180deg,#6b0000 0%,#3a0800 55%,${G.bg} 100%)`,padding:"48px 20px 32px",textAlign:"center"}}>
        ...
        <div style={{fontSize:10,color:"#333",marginTop:12}}>v{VERSAO}</div>
      </div>
    </div>
  );
  ...
  if(view==="login-tel") return(
    <div style={app}>
      ...
      <LoginTel onNext={(tel,cli)=>{setAuthTel(tel);if(cli){setView("login-senha");}else{setCadForm({nome:"",rua:"",num:"",bairroSel:"",senha:"",senhaConf:"",dica:""});setView("login-cad");}}} onBack={()=>setView("splash")}/>
    </div>
  );
    ...
    return(
      <div style={app}>
        ...
        <div style={{padding:"30px 20px",textAlign:"center"}}>
          ...
          <input type="password" style={inp} placeholder="Sua senha" value={senhaInput} onChange={e=>setSenhaInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){
            if(!cli){setSenhaErr("Cliente não encontrado.");return;}
            if(cli.senhaResetada){setCliente(cli);setView("nova-senha");return;}
            if(senhaInput!==cli.senha){setSenhaErr("Senha incorreta.");return;}
            loginSuccess(cli,authTel);setSenhaInput("");setSenhaErr("");setMostrarDica(false);setView("home");
          }}}/>
  ...
  if(view==="login-cad") return(
    <div style={app}>
      ...
      <AuthCadastro form={cadForm} setForm={setCadForm} onSave={()=>{
        ...
        upC([novo,..._clientes]);
        loginSuccess(novo,authTel);
        setView("home");
      }} onBack={()=>setView("login-tel")}/>
    </div>
  );

  // NOVA SENHA
  if(view==="nova-senha") return(
    <div style={app}>
      ...
      <div style={{padding:"24px 20px"}}>
        ...
        <label style={lbl}>Confirme a senha *</label>
        <input type="password" style={inp} placeholder="Digite novamente" value={novaSenhaConf} onChange={e=>setNovaSenhaConf(e.target.value)}/>
        {novaSenhaInput&&novaSenhaConf&&<div style={{fontSize:12,marginBottom:8,color:novaSenhaInput===novaSenhaConf?"#6f6":"#f55"}}>{novaSenhaInput===novaSenhaConf?"✅ Senhas coincidem":"❌ Senhas não coincidem"}</div>}
        <div style={{background:"#1a2a1a",borderRadius:10,padding:"10px",marginBottom:10,fontSize:12,color:"#aaa",border:"1px solid #2d5a2d"}}>💡 Use algo fácil de lembrar.</div>
        <label style={lbl}>Dica para lembrar *</label>
        ...
        <button style={btnG} onClick={()=>{
          ...
          upC(n);loginSuccess({...cliente,senha:novaSenhaInput,dica:novaDicaInput,senhaResetada:false},authTel);
  ...
  if(view==="home") return(
    <div style={app}>
      <div style={hdr}>
        ...
        <button style={{...btnBack,fontSize:12}} onClick={()=>{setCliente(null);setPedidoConf(null);setCart([]);dbSet("elchai_sessao",null);setView("splash");}}>Sair</button>
      </div>
      <div style={{background:`linear-gradient(180deg,#6b0000 0%,#3a0800 60%,${G.bg} 100%)`,padding:"24px 20px 20px",textAlign:"center"}}>
        ...
        <button style={{...btnG,fontSize:17,padding:"18px",marginBottom:12}} onClick={()=>setView("cardapio")}>🍽️ Fazer Pedido</button>
        <button style={{...btnR,fontSize:16,padding:"16px",position:"relative"}} onClick={()=>setView("acompanhar")}>
          📍 Acompanhar Pedido
          {ultimoPedido&&ultimoPedido.status<3&&<span style={{position:"absolute",top:10,right:14,background:G.gl,color:"#000",borderRadius:20,padding:"2px 8px",fontSize:11,fontWeight:800}}>{ST_EMOJI[ultimoPedido.status]}</span>}
        </button>
        {ultimoPedido&&ultimoPedido.status<3&&(
          <div style={{background:G.bc,borderRadius:12,padding:"12px 14px",border:`1px solid ${G.gold}33`,textAlign:"left",marginTop:4}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontWeight:800,color:G.gl}}>#{ultimoPedido.id}</div><div style={{fontSize:12,color:"#ccc"}}>{ultimoPedido.itens.length} item(ns) · {fmt((ultimoPedido.total||0)+(ultimoPedido.frete||0))}</div></div>
  ...
  if(view==="acompanhar"){
    ...
    if(!p) return(
      <div style={app}>
        <div style={hdr}><button style={btnBack} onClick={()=>setView("home")}>← Home</button><span style={{fontWeight:900,fontSize:17,background:gg,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Meu Pedido</span><div/></div>
    ...
    return(
      <div style={app}>
          ...
          {steps.map((step,i)=>{
            ...
            return(
              <div key={i} style={{display:"flex",gap:14,position:"relative"}}>
                {i<steps.length-1&&<div style={{position:"absolute",left:19,top:40,width:2,height:50,background:done?G.gl:`${G.gold}22`,zIndex:0}}/>}
                <div style={{width:40,height:40,borderRadius:"50%",background:done||active?gg:G.bc2,border:`2px solid ${done||active?G.gl:G.gold+"33"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0,zIndex:1,boxShadow:active?`0 0 14px ${G.gl}88`:"none"}}>
                  {done?"✅":step.icon}
                </div>
        ...
        {p.status===3&&<div style={{background:"#1a3a1a",borderRadius:16,padding:"18px",margin:"0 14px 14px",border:"1px solid #2d7a2d",textAlign:"center"}}>
    ...
    return(
      <div style={app}>
        ...
        {tab==="clientes"&&<div style={{padding:"10px 0"}}>
          ...
          {clientes.map((c,i)=><ClienteCard key={i} c={c} upC={upC}/>)}
        </div>}
  ...
  return <div style={{...app,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{color:G.tm}}>Carregando...</div></div>;
}

function LoginTel({onNext,onBack}){
